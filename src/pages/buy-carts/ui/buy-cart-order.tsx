...
export default function OrderDetailsPage() {
    ...
    useEffect(() => {
        (async () => {
            const response = await fetch(`/api/buy-carts/buy-carts-order/${params.id}/order`);
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className='space-y-2'>
                            <h3 className="text-lg font-semibold mb-2">Информация о заказчике</h3>
                            <p> {order.user.name + order.user.lastName}</p>
                            <p>Электронная почта {order.user.email}</p>
                            <p>Номер телефона {order.user.phone ? order.user.phone : 'Не указан'}</p>
                        </div>
                        <div>
                    ...
                    <div className="mt-6 text-right">
                        <p className="text-lg font-semibold">Сумма заказа: ${order.orderDetails.total.toFixed(2)}</p>
                    </div>
                </CardContent>
            </Card>
        </div>
